...
class IDEAS_Feeder(object):
    ...
    def output(self):
        ...
        for i in range(self.nBui): # loop over all households
            ...
            # This is only meaningful if the occupancy profiles are converted first to metabolic heat gains before averaging.
                if resolution==900:
                    for i in range(0,len(QMetOneBuilding_opt[list])-1,2):
                        j=int(3/2*i)
                        QMetOneBuilding_opt[list,i]=2/3*QMetOneBuilding[list,j]+1/3*QMetOneBuilding[list,j+1]
                        QMetOneBuilding_opt[list,i+1]=1/3*QMetOneBuilding[list,j+1]+2/3*QMetOneBuilding[list,j+2]
                    QMetOneBuilding_opt[list,-1] = QMetOneBuilding[list,-1]
                else: # resolution of x*600 seconds (with x a positive integer)
                    QMetOneBuilding_opt[list]=np.append(QMetOneBuilding[list,0:-1].reshape(-1,int(resolution/600)).mean(axis=-1),QMetOneBuilding[list,-1])  # moving average: mean of every x points
            if len(occ)!=0:
            ...
            if len(QMet_opt)!=0:
                QMetOneBuilding_opt.resize(QMetOneBuilding_opt.shape[0],1,QMetOneBuilding_opt.shape[1])
            ...
            for variable in variables.keys(): #loop over variables
                ...
                if len(var)==525601: # if per minute data, make it per 60 min (mDHW, P, Q, QCon, QRad)
                    ...
                    var_opt=np.append(var[0:-1].reshape(-1,int(resolution/60)).mean(axis=-1),var[-1]) # moving average: mean of every 60 points
                if len(var)==52561: # if per 10 minute data, make it per 60 min (sh_day, sh_night, sh_bath)
                    if resolution==900:
                        ...
                        # first point is the existing one for minute 0 (midnight), and the following are averages of previous 60 minutes
                        var_opt = np.append(var[0:-1].reshape(-1,int(resolution/600)).mean(axis=-1),var[-1])  # moving average: mean of every 6 points
        ...
        tim_opt = np.linspace(0, 31536000, int(1+31536000/resolution)) # create time column (always annual simulation=default)
        for variable in variables.keys():
            print(variable)
